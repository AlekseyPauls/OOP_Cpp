...
namespace gol {
    ...
    std::string my_invalid_argument::get_info(const std::string command) {
        ...
        if (command == "step") {
            ...
            res = "Expected one positive integer number - delay in milliseconds. Recommended delay from 100 to 3000. "
                    "For example: 'auto_game 1000'\n\n";
        } else if (command == "save") {
            res = "Can not save file with this name\n\n";
        } else if (command == "load") {
            res = "Can not load file with this name\n\n";
        } else if (command == "help" || command == "start" || command == "back" || command == "reset") {
